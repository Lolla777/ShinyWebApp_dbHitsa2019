                      ...
                      tabItems(
                        ...
                        tabItem(tabName = "select",
                                ...
                                fluidRow( box(width = 6,
                                              downloadButton("downloadDataSelect","Download"),br(),br(),
                                              DTOutput("tabelK")),
                                          box( width = 6,htmlOutput("summary")))),
...
server <- function(input, output,session) {
  ...
  inputPlot41 <- reactive({
    ...
    colnames(df2) <- c("date1","Sensor_1","date2","Sensor_2")
    p <- ggplot(df2,aes(x=Sensor_1, y=Sensor_2)) +
      geom_point(color="deepskyblue3", size=2) +
      labs(title=paste("Plot")) +xlab(input$var2)+ylab(input$var3)+theme(plot.title=element_text(hjust = 0.5,size=14, face="bold"))
    p <- ggplotly(p)
    p <- config(p,modeBarButtons = list(list("toImage")))
    p 
  })
